...
def _init_managers():
	...
	global book_manager, user_manager, admin_manager
	if admin_manager is None:
		from app import DAO
		book_manager = BookManager(DAO)
		user_manager = UserManager(DAO)
		admin_manager = AdminManager(DAO)
	return book_manager, user_manager, admin_manager

# Wrapper decorators
def admin_login_required(f):
	@wraps(f)
	def decorated_function(*args, **kwargs):
		bm, um, am = _init_managers()
		if not am.admin.isLoggedIn():
			return redirect("/admin/signin/")
		return f(*args, **kwargs)
	return decorated_function

def admin_redirect_if_login(f):
	@wraps(f)
	def decorated_function(*args, **kwargs):
		bm, um, am = _init_managers()
		if am.admin.isLoggedIn():
...
def signin():
	...
	if request.method == 'POST':
		...
		d = am.signin(email, hash(password))
...
def books():
	...
	id = int(am.admin.uid())
	admin = am.get(id)
	mybooks = bm.list(availability=0)
...
def book_edit(id):
	...
	if id != None:
		...
		if b and len(b) <1:
			return render_template('books/edit.html', error="No book found!")
